...
def matrix_exp(theta, u):
    ...
    # print("u = {0}".format(u))
    # print("u shape = {0}".format(np.shape(u)))
...
class crossrod:
    def __init__(self, T, dt, total_length, elements, density, radius, total_external_force,
        ...
        self.directors[:,:,1] = np.array(([-np.sin(45), np.cos(45), 0.0],
                                          [0.0, 0.0, 1.0],
                                          [np.cos(45), np.sin(45), 0.0]))
        # print("1st director = {0}".format(self.directors[:,:,0]))
        # print("2nd director = {0}".format(self.directors[:,:,1]))
        ...
        # kappa here
        self.kappa = np.zeros((3,self.n_i)) # NOT SURE IF THIS SHAPE IS CORRECT
        #print("initialized kappa = {0}".format(self.kappa))
        for idx in range(self.n_i):
            # print("matrix_log operation shape = {0}".format(np.shape(matrix_log(np.matmul(self.directors[:,:,(idx+1)],np.transpose(self.directors[:,:,idx]))))))
            ...
            # print("shape of kappa = {0}".format(np.shape(self.kappa)))
        ...
        for x in np.arange(0,T+dt,dt):
            ...
            # print(self.pos)
            # print(self.pos[1,1])
            self.mid_pos.append(self.pos[1,1])
            # if x == 10*dt:
            #     plt.plot(np.arange(0,11*dt,dt),self.mid_pos)
            #     plt.show()
            #     sys.exit()
    ...
    def angular_verlet(self, dt, Q, w):
        ...
        # print("norm_w = {0}".format(norm_w))
        for idx in range(self.e):
            temp_Q[:,:,idx] = matrix_exp((-dt/2),(w[:,idx]/(norm_w[idx]+1E-14))) * Q[:,:,idx]
        ...
        for idx in range(self.e):
            Q_n[:,:,idx] = matrix_exp((-dt/2),(w_n[:,idx]/(norm_w_n[idx]+1E-14))) * temp_Q[:,:,idx]
    ...
    def force_rule(self, temp_pos):
        ...
        # print("internal_force shape = {0}".format(np.shape(self.internal_force)))
    ...
    def bend_rule(self, temp_Q):
        ...
        # print("dJ shape = {0}".format(np.shape(self.dJ)))
        # print("dJ = {0}".format(self.dJ))
    ...
    def bend_update(self, temp_Q):
        ...
        for idx in range(self.n_i):
            # print("first Q_temp = {0}".format(temp_Q[:,:,1]))
            # print("second Q_temp = {0}".format(np.transpose(temp_Q[:,:,0])))
            # print("what you put in the matrix log = {0}".format(np.matmul(self.directors[:,:,(idx+1)],np.transpose(self.directors[:,:,idx]))))
...
plt.plot(np.arange(0,T+dt,dt),test.mid_pos)
plt.show()
