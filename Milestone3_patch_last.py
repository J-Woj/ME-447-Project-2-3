...
class crossrod:
    def __init__(self, T, dt, total_length, elements, density, radius, total_external_force,
                 G = 1E4, E = 1E5, dim = 3, dissipation = 0.1, **kwargs):
        ...
        # Domain
        # UPDATE THIS AT EVERY TIME STEP
        self.domain = (self.l_mag[1:] + self.l_mag[:-1])/2

        #Ref Domain
        # DO NOT UPDATE THIS AT EVERY TIME STEP
        self.domain_hat = (self.l_ref_mag[1:] + self.l_ref_mag[:-1])/2
        ...
        self.directors[0, 2, :] = 1
        self.directors[1, 1, :] = -1
        self.directors[2, 0, :] = 1

        self.forces = np.zeros((dim,self.n)) # forces INITIALIZE
        self.forces[1,-1] = -total_external_force
        ...
        # Internal dissipation Forces and Couples
        self.dissipation = dissipation
        self.dissipation_force = -self.dissipation * self.vel
        self.dissipation_couple = -self.dissipation * self.ang_vel
    ...
    def step(self, dt):
        self.pos, self.directors, self.vel, self.ang_vel = self.verlet(dt, self.pos, self.directors, self.vel, self.ang_vel)
        self.update(temp_pos = self.pos, temp_Q = self.directors)
        self.time_elapsed += dt
        pass

    def verlet(self, dt, x, Q, v, w):
        # top of page 3 of Project 2 document, equations done in order
        temp_x = x + 0.5*dt*v
        ...
        return x_n, Q_n, v_n, w_n

    def force_rule(self, temp_pos):
        # First update
        self.update(temp_pos = temp_pos)
        ...
        dv_dt = (self.internal_force + self.forces + self.dissipation_force)  / self.m
        return dv_dt

    def bend_rule(self, temp_Q):
        # First update
        self.update(temp_Q = temp_Q)

        # Governing equation 4 with assumptions
        B_kappa = np.zeros((3,self.n_i))
        B_kappa = np.einsum('ijk,jk->ik',self.B,self.kappa)

        termOne = B_kappa / self.epsilon**3

        termTwo = np.zeros((3,self.n_i))
        for idx in range(self.n_i):
            termTwo[:,idx] = (np.cross(self.kappa[:,idx],B_kappa[:,idx]) / self.epsilon[idx]**3) * self.domain_hat[idx]

        self.bend_twist_internal_couple = grad_h(termOne, self.e) + A_h(termTwo, self.e) # Governing equation 4 with assumptions
        ...
        for idx in range(self.e):
            dw_dt[:,idx] = np.matmul(np.linalg.inv(self.dJ[:,:,idx]),self.dil_fac[idx]*(self.bend_twist_internal_couple[:,idx] + self.shear_stretch_internal_couple[:,idx])+self.dissipation_couple[:,idx])
        return dw_dt

    def update(self, temp_pos = [], temp_Q = []):
        # Setting parameters depending on which ones were passed
        if not np.any(temp_pos):
            temp_pos = self.pos
        if not np.any(temp_Q):
            temp_Q = self.directors

        # Constrain 1st and last node position and velocity for Milestone 1 & 3
        temp_pos[:,0] = 0
        self.vel[:,0] = 0
        temp_Q[:,:,0] = np.eye(3)
        ...
        # Update internal dissipation Forces and Couples
        self.dissipation_force = -self.dissipation * self.vel
        self.dissipation_couple = -self.dissipation * self.ang_vel
        ...
        for idx in range(self.n_i):
            ...
            # print("l_mag = {0}".format(self.l_mag))
...
# test = A_h([1,2,3,4,5,6,7])
# print(test)

timBeam = crossrod(T = T, dt = dt, total_length = L, elements = 20, density = 5E3, radius = R, total_external_force = F)

y_pos_end = []

# for x in np.arange(0,T+dt,dt):
#     timBeam.step(dt)
#     y_pos_end.append(timBeam.pos[1,-1])

y = -(F*L)/(shape_factor*A*G)-(F*L**3)/(2*E*timBeam.I[1,1,1])+(F*L**3)/(6*E*timBeam.I[1,1,1])

# print("real y = {0}".format(y))
# print("sim y = {0}".format(np.average(y_pos_end)))
...
fig = plt.figure()
ax = fig.add_subplot(111, aspect='equal', autoscale_on=False, xlim=(0, 3), ylim=(-1, 1))
ax.grid()
...
def animate(i):
    ...
    line.set_data(timBeam.pos[2,:],timBeam.pos[1,:])
    time_text.set_text('time = %.1f' % timBeam.time_elapsed)
    return line, time_text
...
anim.save('C:/Users/wojta/Desktop/Fall 2021/ME 447/Project 2/milestone3animation.gif', writer='Pillow', fps=60)
