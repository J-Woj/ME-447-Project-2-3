...
class crossrod:
    def __init__(self, T, dt, total_length, elements, density, radius, total_external_force,
        ...
        self.directors[0, 0, :] = 1
        self.directors[1, 1, :] = 1
        self.directors[2, 2, :] = 1
    ...
    def bend_rule(self, temp_Q):
        ...
        for idx in range(self.e):
            dw_dt[:,idx] = np.matmul(np.linalg.inv(self.dJ[:,:,idx]),self.dil_fac[idx]*(self.bend_twist_internal_couple[:,idx] + self.shear_stretch_internal_couple[:,idx]))
        return dw_dt
...
# anim.save('C:/Users/wojta/Desktop/Fall 2021/ME 447/Project 2/animation.gif', writer='imagemagick', fps=60)
