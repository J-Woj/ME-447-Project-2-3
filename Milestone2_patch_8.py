...
class crossrod:
    ...
    def angular_verlet(self, dt, Q, w):
        ...
        for idx in range(self.e):
            temp_Q[:,:,idx] = matrix_exp((-dt/2)*(180/np.pi)*norm_w[idx],(w[:,idx]/(norm_w[idx]+1E-14))) * Q[:,:,idx]
        ...
        for idx in range(self.e):
            Q_n[:,:,idx] = matrix_exp((-dt/2)*(180/np.pi)*norm_w_n[idx],(w_n[:,idx]/(norm_w_n[idx]+1E-14))) * temp_Q[:,:,idx]
...
def animate(i):
    ...
    time_text.set_text('time = %.1f' % test.time_elapsed)
    return line, time_text
